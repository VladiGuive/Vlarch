...
# Syncs every top-level path under dotfiles/; never mirrors or deletes all of $HOME.
...
_vlarch_deploy_dotfiles_config() {
  local user="$1" src_config="$2" home="$3"
  local entry base
  ...
  done < <(find "$src_config" -mindepth 1 -maxdepth 1 -print0)
...
vlarch_deploy_dotfiles() {
  ...
  while IFS= read -r -d '' entry; do
    base="$(basename "$entry")"
    if [[ -f "$entry" ]]; then
      install -Dm0644 -o "$user" -g "$user" "$entry" "${home}/${base}"
    elif [[ -d "$entry" ]]; then
      case "$base" in
        .config)
          _vlarch_deploy_dotfiles_config "$user" "$entry" "$home"
          ;;
        .local | .hermes | *)
          mkdir -p "${home}/${base}"
